...
def runbook_check(runbook):
    if runbook in runbook_mapping.keys():
        return runbook_mapping[runbook]
    else:
        return "no runbook exist"
...
def lambda_handler(event, context):
    ...
    if 'runbook' in alarm_details:
        runbook_key = alarm_details['runbook']
    else:
        runbook_key = ''
    ...
    alert_id = alert_source + '-' + element + '-' + element_type + '-' + element_subtype
    ...
    if message.get('AlarmName'):
        ...
        attachments = [{
            ...
            "title_link": alert_mapping.get(alert_id, ''), 
            "fields": [{
                "title": "Status",
            ...
            {
                "title": "runbook",
                "value": '```' + runbook_check(runbook_key) + '```',
                "short": False
            }
            ],
